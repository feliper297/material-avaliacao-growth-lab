...
Esta é a versão adaptada para instalação local via Codex, sem depender de nenhum serviço
...
Codex acessível nesta sessão (ver `.Codex/skills/pixel/SKILL.md` e
`.Codex/skills/pixel-monster/SKILL.md`).
...
2. **Visual ou background** — perguntar SEMPRE antes de qualquer teste de browser (Playwright via
   `.Codex/skills/pixel-test/SKILL.md`):
...
O hook `.Codex/hooks/pixel-visual-ask.js` (UserPromptSubmit) injeta um lembrete quando detecta
...
`.Codex/skills/pixel-test/SKILL.md` para abrir com Playwright e capturar screenshot full-page +
...
- Sem tokens/segredos reais em relatórios — o hook `.Codex/hooks/secret-scan-gate.js` bloqueia
